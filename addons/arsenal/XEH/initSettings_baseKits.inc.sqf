...
private _base_configs = Q(configName _x isNotEqualTo QQ(Base)) configClasses (configFile >> QGVAR(baseKits));

{
    ...
    [
        [ADDON, "baseKit", _configName] joinString "_",	//    _setting     - Unique setting name. Matches resulting variable name <STRING>
        "CHECKBOX",								        //    _settingType - Type of setting. Can be "CHECKBOX", "EDITBOX", "LIST", "SLIDER" or "COLOR" <STRING>
        ...
        1,										        //    _isGlobal    - 1: all clients share the same setting, 2: setting can't be overwritten (optional, default: 0) <NUMBER>
        {
            // [_configName, _this] call FUNC(setBaseKit);
        },										        //    _script      - Script to execute when setting is changed. (optional) <CODE>
        false									        //    _needRestart - Setting will be marked as needing mission restart after being changed. (optional, default false) <BOOL>
    ] call CBA_fnc_addSetting;
